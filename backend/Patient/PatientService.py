...
def createPatient():
    ...
    if request.method == 'POST':
        ...
        if (content_type == 'application/json'):
            ...
            isPatient = session.query(Patient).filter(
                Patient.user_email == user_email).first()
            ...
            id_patient = session.query(Patient.id_patient).filter(
                Patient.user_email == user_email).first()

            # Health Details added here. Use Standard Healthy person value
            patientInfo = session.query(Patient).get(id_patient)
            patient_health_details = HealthDetailsModel.HealthDetails(datetime.now(
            ), 'unknown', 37, '120/120', '30', 9.0, '90', 70.5, 33.2, patientInfo.id_patient)
...
def getDoctorByPatientId():
    ...
    try:
        ...
        doctor = session.query(Doctor).join(Patient, Doctor.id_doctor == Patient.id_doctor).filter(
            Patient.id_patient == id_patient).first()
        returnInfo = {
            'msg': {
