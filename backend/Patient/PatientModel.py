...
class Patient(Base):
    ...
    id_patient = Column(Integer, primary_key=True, unique=True,
                        autoincrement=True, nullable=False)
    id_guardian = Column(Integer, ForeignKey(
        GuardianPerson.id_guardian, ondelete="SET NULL"), nullable=True)
    id_doctor = Column(Integer, ForeignKey(
        Doctor.id_doctor, ondelete="SET NULL"))
    ...
    # relationships
    health_details = relationship(
        "HealthDetails", uselist=False, back_populates="patient")
    prescription = relationship(
        "Prescription", cascade='save-update, merge, delete')
    appointments = relationship("Appointment", backref="patient")

    def __init__(self, first_name, last_name, user_email, user_password, date_of_birth, person_image, id_guardian=None, id_doctor=None):
        ...
        self.id_guardian = id_guardian if id_guardian is not None else None
