...
def getReports():
    ...
    try:
        reports = session.query(Report.id_report, Report.report_url, Report.report_type, Report.description, Report.id_patient,
                                Report.created_at, Doctor.first_name, Doctor.last_name).join(Doctor).filter(Report.id_doctor == Doctor.id_doctor).all()
        Json_reports = [{
            "status": True,
            "msg": {
...
def getReportByPatientId(id_patient):
    from app import session
    try:
        reports = session.query(Report, Doctor).join(
            Doctor, Report.id_doctor == Doctor.id_doctor).filter(Report.id_patient == id_patient).all()
        report_info = []
...
def createReport():
    ...
    if content_type == 'application/json':  # check if content is in json format
        ...
        new_report = Report(report_type=report_type, report_url=report_url, description=description,
                            id_patient=id_patient, id_doctor=id_doctor, created_at=upload_date)
