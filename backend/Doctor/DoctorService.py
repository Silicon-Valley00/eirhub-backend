...
def createDoctor():
    ...
    if request.method == 'POST':
        ...
        if (content_type == 'application/json'):
            ...
            try:
                doctorExists = session.query(Doctor).filter(
                    Doctor.user_email == user_email).first()
                if (doctorExists):
                    return ({
                        'status': False,
                        'msg': {
                            "message": "Doctor already registered. Do you want to login?"
                        }
                    }), 200
            except Exception as e:
                return ({
            ...
            # gender = req["gender"]
            hospital_code = req["hospital_code"]
            hospital_name = session.query(Hospital.hospital_name).filter(
                Hospital.hospital_code == req["hospital_code"]).scalar()

            # hash password
            hashed_password = generate_password_hash(user_password)
            newDoctor = Doctor(first_name=first_name, last_name=last_name, user_email=user_email, user_password=hashed_password,
                               date_of_birth=date_of_birth, hospital_code=hospital_code, hospital_name=hospital_name, person_image=person_image)
            try:
            ...
            id_doctor = session.query(Doctor.id_doctor).filter(
                Doctor.user_email == newDoctor.user_email).first()
        ...
        if (content_type == 'application/json'):
            ...
            try:
                ...
                if (id_doctor):
                    ...
                    session.commit()
                    patients = session.query(Patient).join(Appointment, Doctor).filter(
                        Doctor.id_doctor == doctorInfo.id_doctor).filter(Appointment.appointment_status != 'Declined').all()
                    session.commit()
...
def updateDoctorById(doctorId):
    ...
    try:
        ...
        doctor.hospital_name = session.query(Hospital.hospital_name).filter(
            Hospital.hospital_code == docReq["hospital_code"]).scalar()
...
def getpatientsByDoctorId():
    ...
    try:
        ...
        patients = session.query(Patient).join(Appointment, Doctor).filter(Doctor.id_doctor == id_doctor).filter(
            Appointment.appointment_status != 'Declined').filter(Appointment.appointment_status != 'Elapsed').all()
        returnInfo = {
            'status': True,
            'msg': [{
                "id_patient": patient.id_patient,
...
def getStatsByDoctorId():
    ...
    try:
        id_doctor = int(request.args.get("id_doctor"))
        number_of_patients = session.query(Patient).join(Appointment, Doctor).filter(
            Doctor.id_doctor == id_doctor, (Appointment.appointment_status == 'Accepted')).distinct().count()
        number_of_reports = session.query(Report,Doctor).join(Doctor).filter(Doctor.id_doctor == id_doctor).count()
...
#         id_doctor = int(request.args.get("id_doctor"))
#         patients = session.query(Patient).join(Appointment, Doctor).filter(
#             Doctor.id_doctor == id_doctor, (Appointment.appointment_status == 'Pending')).all()
...
#         number_of_appointments = session.query(Appointment).filter(
#             Appointment.id_doctor == id_doctor).count()
#         returnInfo = {
#             'status': True,
#             'msg': {
#                 'patients': [
